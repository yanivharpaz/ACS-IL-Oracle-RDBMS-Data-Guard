...
prep_dg_01()
{
    ...
    then
        ...
        $SU -s /bin/bash  $ORACLE_OWNER -c "$SQLPLUS -s /nolog << EOF
            ...
            ALTER DATABASE ADD STANDBY LOGFILE ('$ORACLE_REDO_LOCATION/standby_redo01.log') SIZE $STANDBY_REDO_LOG_SIZE;
            ALTER DATABASE ADD STANDBY LOGFILE ('$ORACLE_REDO_LOCATION/standby_redo02.log') SIZE $STANDBY_REDO_LOG_SIZE;
            ALTER DATABASE ADD STANDBY LOGFILE ('$ORACLE_REDO_LOCATION/standby_redo03.log') SIZE $STANDBY_REDO_LOG_SIZE;
            ALTER DATABASE ADD STANDBY LOGFILE ('$ORACLE_REDO_LOCATION/standby_redo04.log') SIZE $STANDBY_REDO_LOG_SIZE;
